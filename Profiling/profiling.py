...
def primes_fast(N):
    ...
    while length < N:
        ...
        for i in primes_list:
            #Primes will never be greater than the square root
            if i > x:
                break
            ...
            if current % i == 0:
                isprime = False
                break
        #Append all the primes to the list
        if isprime:
            primes_list.append(current)
            length += 1
        current += 2
...
def name_scores_fast(filename='names.txt'):
    ...
    #Initialize data here to minimize look-ups
    total = 0
    alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
def fibonacci_digits(N=1000):
    ...
    #Get indices and fibonacci numbers
...
def prime_sieve(N):
    ...
    #np method is much faster than range()
...
#I hope this works but @jit is not working on my computer
@jit
